...
test("接口: destroy",function(){
    ua.destroyTest(function(w,f){
    	var dl1 = w.dt.domLength(w);
        var el1= w.dt.eventLength();

        w.$("body").append('<div id="luckchest"></div>');
        w.$("body").append('<div id="prize_1">prize</div>');
        
        var luckchest = w.$.ui.luckydrawbox("#luckchest",{
        	result:"#prize_1"
        });
        ...
        $.later(function(){
            ...
            var ol = w.dt.objLength(luckchest);
            var dl2 =w.dt.domLength(w);

            equal(dl1,dl2,"The dom is ok");  //fix影响
...
test("交互：根据用户的点击结果调用setResult",function(){
    stop();
    expect(2);
